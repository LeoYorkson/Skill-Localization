...
class graft_Trainer(nn.Module):
    ...
    def solve_v(self):
        ...
        k = num_params * sparsity_level
        a, b = 0, 0
        for counter in range(len(self.trainable_name)):
            b = max(b,self.trainable_parameters[counter].max())
        def f(v):
            s = 0
            for counter in range(len(self.trainable_name)):
                s += (self.trainable_parameters[counter] - v).clamp(0, 1).sum()
            return s - k
        if f(0) > 0:
            return 0
        itr = 0
    ...
    ##################################################################################################################
    def constrainScoreByWhole(self):
        total = 0
        for counter in range(len(self.trainable_name)):
